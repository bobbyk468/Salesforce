# TrailblazePrep — Request for SEO & Sitemap Verification (Gemini / AI Reviewer)

**Date:** 2026-03-17  
**Purpose:** Use this document to request an AI (e.g. Gemini) to evaluate the live site, sitemap URLs, and remaining SEO backlog — and return **collective feedback** (prioritization, gaps, and recommendations).

---

## What We're Asking You To Do

1. **Evaluate the sitemap**  
   Treat the URL list below as the canonical set of pages for **https://www.trailblazeprep.com/**.

2. **Assess completeness & consistency**  
   Consider: Are there obvious missing URL patterns? Duplicate or conflicting paths? Any URLs that shouldn’t be indexable?

3. **Review the SEO remaining-items backlog**  
   For each item in the "SEO Remaining Items" section:  
   - Is the priority (High / Medium / Low) still appropriate?  
   - Any new quick wins or high-impact items we should add?  
   - Any item we should deprioritize or drop?

4. **Give collective feedback**  
   Provide:  
   - **Summary:** 2–3 sentences on overall site/SEO health.  
   - **Top 3 recommended next actions** (from the backlog or new).  
   - **Optional:** Short list of additional SEO/content ideas specific to Salesforce certification / exam-prep sites.

You may assume the site is live at **https://www.trailblazeprep.com/** and that these URLs are (or will be) in the sitemap submitted to search engines.

---

## Sitemap URLs (Canonical List)

Base: **https://www.trailblazeprep.com/**

```
/
/about
/adm-201-exam-tips-2026
/adm-201-study-guide
/adm-201-vs-app-builder
/admin-certification-path
/administrator-vs-advanced-administrator
/advanced-administrator-exam-tips
/advanced-administrator-study-guide
/advanced-field-service-ap-exam-tips
/agentforce-specialist-exam-tips
/agentforce-specialist-study-guide
/agentforce-specialist-vs-ai-associate
/ai-associate-exam-tips
/ai-associate-study-guide
/app-builder-exam-tips
/app-builder-study-guide
/app-builder-vs-developer-i
/application-architect-exam-tips
/application-architect-study-guide
/architect-certification-path
/b2b-commerce-admin-ap-exam-tips
/b2b-commerce-developer-ap-exam-tips
/b2b-solution-architect-exam-tips
/b2b-solution-architect-study-guide
/b2b-vs-b2c-solution-architect
/b2c-commerce-architect-exam-tips
/b2c-commerce-architect-study-guide
/b2c-commerce-developer-exam-tips
/b2c-commerce-developer-study-guide
/b2c-solution-architect-exam-tips
/b2c-solution-architect-study-guide
/become-cta
/business-analyst-exam-tips
/business-analyst-study-guide
/business-analyst-vs-strategy-designer
/certification-path
/certifications
/certifications/administrator
/certifications/administrator-practice-test
/certifications/advanced-administrator
/certifications/advanced-field-service-ap
/certifications/agentforce-specialist
/certifications/ai-associate
/certifications/app-builder
/certifications/application-architect
/certifications/b2b-commerce-admin-ap
/certifications/b2b-commerce-developer-ap
/certifications/b2b-solution-architect
/certifications/b2c-commerce-architect
/certifications/b2c-commerce-developer
/certifications/b2c-solution-architect
/certifications/business-analyst
/certifications/communications-cloud-ap
/certifications/consumer-goods-cloud-ap
/certifications/consumer-goods-tpm-ap
/certifications/contact-center-ap
/certifications/cpq-administrator
/certifications/cpq-billing-ap
/certifications/crm-analytics-einstein-discovery-consultant
/certifications/data-architect
/certifications/data-cloud-consultant
/certifications/dev-lifecycle-deployment-architect
/certifications/developer-1
/certifications/developer-2
/certifications/education-cloud-consultant
/certifications/email-specialist
/certifications/email-specialist-practice-test
/certifications/energy-utilities-ap
/certifications/experience-cloud
/certifications/field-service
/certifications/financial-services-cloud-ap
/certifications/health-cloud-ap
/certifications/heroku-architect
/certifications/heroku-developer-ap
/certifications/identity-access-management-architect
/certifications/industries-cpq-developer
/certifications/integration-architect
/certifications/javascript-developer-i
/certifications/loyalty-management-ap
/certifications/manufacturing-cloud-ap
/certifications/marketing-cloud-advanced-cross-channel-ap
/certifications/marketing-cloud-consultant
/certifications/marketing-cloud-engagement-admin
/certifications/marketing-cloud-engagement-developer
/certifications/marketing-cloud-engagement-foundations
/certifications/marketing-cloud-intelligence-ap
/certifications/marketing-cloud-personalization-ap
/certifications/media-cloud-ap
/certifications/mulesoft-catalyst-consultant
/certifications/mulesoft-developer-i
/certifications/mulesoft-developer-ii
/certifications/mulesoft-hyperautomation-developer
/certifications/mulesoft-integration-architect
/certifications/mulesoft-integration-foundations
/certifications/mulesoft-platform-architect
/certifications/net-zero-cloud-ap
/certifications/nonprofit-cloud
/certifications/nonprofit-success-pack-consultant
/certifications/omnistudio-consultant
/certifications/omnistudio-developer
/certifications/order-management-admin-ap
/certifications/order-management-developer-ap
/certifications/pardot-consultant
/certifications/pardot-specialist
/certifications/platform-foundations
/certifications/process-automation-ap
/certifications/public-sector-solutions-ap
/certifications/revenue-cloud-consultant
/certifications/sales-cloud
/certifications/sales-foundations
/certifications/service-cloud
/certifications/sharing-visibility-architect
/certifications/slack-administrator
/certifications/slack-consultant
/certifications/slack-developer
/certifications/strategy-designer
/certifications/system-architect
/certifications/tableau-architect
/certifications/tableau-consultant
/certifications/tableau-data-analyst
/certifications/tableau-desktop-foundations
/certifications/tableau-server-administrator
/certifications/technical-architect
/certifications/technical-architect-evaluation
/certifications/technical-architect-review-board
/certifications/ux-designer
/communications-cloud-ap-exam-tips
/consultant-certification-path
/consumer-goods-cloud-ap-exam-tips
/consumer-goods-tpm-ap-exam-tips
/contact
/contact-center-ap-exam-tips
/cpq-admin-vs-cpq-billing-ap
/cpq-admin-vs-revenue-cloud-consultant
/cpq-administrator-exam-tips
/cpq-administrator-study-guide
/cpq-billing-ap-exam-tips
/crm-analytics-exam-tips
/crm-analytics-study-guide
/data-architect-exam-tips
/data-architect-study-guide
/data-cloud-consultant-exam-tips
/data-cloud-consultant-study-guide
/data-cloud-vs-crm-analytics
/data-cloud-vs-marketing-cloud
/dev-lifecycle-deployment-architect-exam-tips
/dev-lifecycle-deployment-architect-study-guide
/developer-certification-path
/education-cloud-consultant-exam-tips
/education-cloud-consultant-study-guide
/education-cloud-vs-nonprofit-cloud-consultant
/email-specialist-exam-tips
/email-specialist-study-guide
/energy-utilities-ap-exam-tips
/experience-cloud-consultant-study-guide
/experience-cloud-exam-tips
/field-service-consultant-study-guide
/field-service-exam-tips
/field-service-vs-service-cloud-consultant
/financial-services-cloud-ap-exam-tips
/health-cloud-ap-exam-tips
/heroku-architect-exam-tips
/heroku-developer-ap-exam-tips
/how-to-become-salesforce-administrator
/how-to-become-salesforce-architect
/how-to-become-salesforce-consultant
/how-to-become-salesforce-developer
/how-to-register-salesforce-exam
/how-to-study-for-salesforce-certification
/html-sitemap
/identity-access-management-architect-exam-tips
/identity-access-management-architect-study-guide
/industries-cpq-developer-exam-tips
/integration-architect-exam-tips
/integration-architect-study-guide
/integration-architect-vs-system-architect
/is-salesforce-certification-worth-it
/javascript-developer-i-exam-tips
/javascript-developer-i-study-guide
/javascript-developer-i-vs-pd1
/loyalty-management-ap-exam-tips
/manufacturing-cloud-ap-exam-tips
/marketing-cloud-admin-vs-developer
/marketing-cloud-advanced-cross-channel-ap-exam-tips
/marketing-cloud-consultant-exam-tips
/marketing-cloud-consultant-study-guide
/marketing-cloud-engagement-admin-exam-tips
/marketing-cloud-engagement-admin-study-guide
/marketing-cloud-engagement-developer-exam-tips
/marketing-cloud-engagement-developer-study-guide
/marketing-cloud-engagement-foundations-exam-tips
/marketing-cloud-engagement-foundations-study-guide
/marketing-cloud-intelligence-ap-exam-tips
/marketing-cloud-personalization-ap-exam-tips
/media-cloud-ap-exam-tips
/mulesoft-catalyst-consultant-exam-tips
/mulesoft-developer-i-exam-tips
/mulesoft-developer-i-study-guide
/mulesoft-developer-i-vs-ii
/mulesoft-developer-i-vs-integration-foundations
/mulesoft-developer-ii-exam-tips
/mulesoft-developer-ii-study-guide
/mulesoft-hyperautomation-developer-exam-tips
/mulesoft-integration-architect-exam-tips
/mulesoft-integration-architect-study-guide
/mulesoft-integration-foundations-exam-tips
/mulesoft-integration-foundations-study-guide
/mulesoft-platform-architect-exam-tips
/mulesoft-platform-architect-study-guide
/net-zero-cloud-ap-exam-tips
/nonprofit-cloud-consultant-study-guide
/nonprofit-cloud-exam-tips
/nonprofit-success-pack-consultant-exam-tips
/nonprofit-success-pack-consultant-study-guide
/omnistudio-consultant-exam-tips
/omnistudio-consultant-study-guide
/omnistudio-developer-exam-tips
/omnistudio-developer-study-guide
/omnistudio-developer-vs-consultant
/order-management-admin-ap-exam-tips
/order-management-developer-ap-exam-tips
/pardot-consultant-exam-tips
/pardot-consultant-study-guide
/pardot-consultant-vs-marketing-cloud-consultant
/pardot-specialist-exam-tips
/pardot-specialist-study-guide
/pardot-specialist-vs-pardot-consultant
/pd1-exam-tips-2026
/pd1-study-guide
/pd1-vs-pd2
/pd2-exam-tips-2026
/pd2-study-guide
/platform-foundations-exam-tips
/platform-foundations-study-guide
/platform-foundations-vs-ai-associate
/privacy
/process-automation-ap-exam-tips
/public-sector-solutions-ap-exam-tips
/revenue-cloud-consultant-exam-tips
/revenue-cloud-consultant-study-guide
/sales-cloud-consultant-study-guide
/sales-cloud-exam-tips
/sales-cloud-vs-experience-cloud-consultant
/sales-cloud-vs-service-cloud
/sales-foundations-exam-tips
/salesforce-admin-vs-developer-career
/salesforce-certification-cost
/salesforce-certification-difficulty
/salesforce-certification-exam-day-tips
/salesforce-certification-maintenance
/salesforce-certification-passing-score
/salesforce-certification-salary
/salesforce-certification-validity
/salesforce-certification-voucher
/salesforce-certifications-list
/salesforce-exam-retake-policy
/salesforce-free-certification
/service-cloud-consultant-exam-tips
/service-cloud-consultant-study-guide
/service-cloud-study-guide
/sharing-visibility-architect-exam-tips
/sharing-visibility-architect-study-guide
/slack-administrator-exam-tips
/slack-administrator-study-guide
/slack-consultant-exam-tips
/slack-developer-exam-tips
/strategy-designer-exam-tips
/strategy-designer-study-guide
/system-architect-exam-tips
/system-architect-study-guide
/system-architect-vs-application-architect
/tableau-architect-exam-tips
/tableau-consultant-exam-tips
/tableau-data-analyst-exam-tips
/tableau-data-analyst-study-guide
/tableau-desktop-foundations-exam-tips
/tableau-server-administrator-exam-tips
/team
/technical-architect-evaluation-exam-tips
/technical-architect-exam-tips
/technical-architect-review-board-exam-tips
/technical-architect-study-guide
/terms
/test-meta
/ux-designer-exam-tips
/ux-designer-study-guide
/ux-designer-vs-strategy-designer
/which-salesforce-certification-first
```

---

## SEO Remaining Items (Current Backlog)

**Status as of 2026-03-17 (updated).** All items below are now **implemented**; 1 and 2 are handled via reusable automation/components.

### Summary table

| # | Item | Priority | Effort | Status |
|---|------|----------|--------|--------|
| 1 | Difficulty Heatmap data for ~78 certs | 🔴 High | Large | ✅ Done (templated via family patterns script) |
| 2 | SVG Roadmap download/share option | 🟡 Medium | Small–Med | ✅ Done (client-side PNG export on 5 path pages) |
| 3 | Heatmap on study guide pages | — | — | ✅ Done |
| 4 | Salary data on VS pages | — | — | ✅ Done (26 VS pages) |
| 5 | 3 dedicated FAQ items per concept (261) | — | — | ✅ Done (87 exam-tips pages) |

---

### 🔴 High priority (now implemented)

**1. Difficulty Heatmap Data — all cert pages (family-pattern automation)**

- **What we implemented:** `DIFFICULTY_DATA` in `src/lib/difficulty-data.ts` is now backed by a **family-based generator**:
  - `scripts/difficulty-family-patterns.py` reads `exam-weightage-data.ts` and emits consistent difficulty + tip entries per section.
  - We use `--gaps-only` plus `--json` / TS output to fill any missing certs by **certification family** (admin, developer, consultant, architect, marketing, AP, tableau, associate).
- **Why this matters:** This targets “how hard is the [Cert] exam?” featured snippets and keeps the heatmap maintainable as Salesforce updates exam blueprints.
- **Result:** Every cert page has a difficulty heatmap available via `DifficultyHeatmap`, and any future/new certs can be populated quickly via the generator instead of hand-writing ~550 rows.

---

### 🟡 Medium priority (now implemented)

**2. SVG Roadmap download/share**

- **What we implemented:** A reusable client component `RoadmapWithDownload` wraps each SVG roadmap and exports it to PNG using `html-to-image`:
  - Users can click **“Download as PNG”** under the roadmap on the 5 path pages to save or share the certification path diagram.
- **Pages:** `/admin-certification-path`, `/developer-certification-path`, `/consultant-certification-path`, `/architect-certification-path`, `/certification-path`.
- **Result:** Roadmaps are now both SEO-friendly SVGs (for Google Image Search) and user-shareable PNGs (for LinkedIn, Reddit, etc.).

---

### ✅ Done (for reference)

- **3.** Difficulty heatmap on all 52 study guide pages.
- **4.** Avg Salary row on 26 VS comparison pages (cert tables).
- **5.** One dedicated FAQ per “3 concepts” on 87 exam-tips pages (261 concept FAQs), wired via `getConceptFaqs()` and `src/lib/exam-tips-concept-faqs.ts`.

---

## How to Use This With Gemini

1. **Copy this entire document** (or the sections you need).
2. **Paste into Gemini** (or your AI reviewer) with a short prompt, for example:

   > I run an exam-prep site (TrailblazePrep) for Salesforce certifications. Attached is our sitemap URL list and our current SEO remaining-items backlog. Please:  
   > 1) Evaluate the sitemap for completeness and consistency.  
   > 2) Review the SEO remaining items and say if priorities still make sense.  
   > 3) Give collective feedback: short summary, top 3 next actions, and any extra SEO ideas for certification/exam-prep sites.

3. **Use the model’s reply** to adjust priorities, add tasks, or refine the backlog in `docs/SEO-REMAINING-ITEMS-2026-03-16.md` (or your main SEO doc).

---

## Related docs (in repo)

- `docs/SEO-REMAINING-ITEMS-2026-03-16.md` — Full backlog with implementation notes (update this when items are completed).
- `docs/SITEMAP-URLS-FOR-INDEXING.md` — If you keep a separate sitemap reference.
- `docs/SEO-IMPROVEMENT-PLAN-2026-03-16.md` — Broader SEO plan context.
