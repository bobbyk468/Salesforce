...
// Tweet 1 was already posted — resume from tweet 2
const firstTweetId = '2045892879168774540';
...
async function main() {
  console.log('\n▶️  Continuing Thread 2 (tweets 2–5)...\n');

  let replyToId = firstTweetId;
  for (let i = 0; i < remainingTweets.length; i++) {
    console.log(`Posting tweet ${i + 2}/5...`);
    const { data } = await client.v2.tweet({
      text: remainingTweets[i],
      reply: { in_reply_to_tweet_id: replyToId },
    });
    replyToId = data.id;
    console.log(`✅ Tweet ${i + 2} posted: https://x.com/i/web/status/${data.id}`);
    if (i < remainingTweets.length - 1) {
