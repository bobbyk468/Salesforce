...
if (queue.some((e) => e.id?.startsWith('gw-bk26-'))) {
  console.error('Already contains gw-bk26-* rows; delete those entries first if you need to re-seed.');
  process.exit(1);
}
...
for (const ymd of ymdRange('2026-06-11', '2026-07-31')) {
  const slots = [
    [`${ymd}T09:45:00-04:00`, 'Tip — exam grind'],
    [`${ymd}T15:45:00-04:00`, 'Tip — platform pattern'],
  ];
  for (const [iso, titlePrefix] of slots) {
    let id = `gw-bk26-${String(seq).padStart(4, '0')}`;
    while (existing.has(id)) {
      seq++;
      id = `gw-bk26-${String(seq).padStart(4, '0')}`;
    }
    existing.add(id);
...
console.log(`Appended ${newRows.length} tips (Jun 11 – Jul 31, 2/day ET). Run: npm run x:tag-cert-focus`);
