...
const pageTitle = `CPQ Specialist Exam Tips (${RELEASE_CURRENT}): Study Guide`
const ogImageUrl = `${siteUrl}/og?t=${encodeURIComponent(pageTitle)}`
const pageDescription =
  `Salesforce CPQ Specialist exam tips for ${RELEASE_CURRENT}: product catalogue, pricing rules, quote configuration, and scenario strategy to pass first attempt.`
...
export default function CPQAdministratorExamTipsPage() {
  const webPageJsonLd = getWebPageJsonLd({ name: pageTitle, description: pageDescription, path: '/cpq-administrator-exam-tips', breadcrumbItems })
  const breadcrumbJsonLd = getBreadcrumbListJsonLd(breadcrumbItems)
  const articleJsonLd = getArticleJsonLd({ headline: pageTitle, description: pageDescription, path: '/cpq-administrator-exam-tips' })
    ...
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <section className="rounded-xl border border-gray-100 bg-white p-6 mb-8">
        ...
        <div className="space-y-3 text-sm text-gray-700">
          ...
          <p className="text-xs text-gray-500 pt-1">Products + Pricing = 54%. Master the price waterfall and bundle options before anything else.</p>
        </div>
      </section>
