...
import type { CertificationPath } from '@/lib/certification-path-data'
...
export default function CertificationPathContent() {
  const [paths, setPaths] = useState<CertificationPath[] | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch('/api/certification-paths')
      .then((res) => res.json())
      .then((data) => {
        setPaths(data)
        setLoading(false)
  ...
  if (loading) {
    return (
      <div className="space-y-16 py-8 text-center text-gray-500">
        <p className="text-sm">Loading certification paths…</p>
      </div>
    )
  }
                  ...
                  <div key={stageIndex} className="relative flex flex-col lg:flex-row lg:items-start gap-6">
                    <div className="lg:w-48 flex-shrink-0">
                      ...
                      {stageGroup.description && (
                        <p className="mt-2 text-sm text-gray-500 lg:max-w-[12rem]">{stageGroup.description}</p>
                      )}
                    </div>
                      ...
                      {stageGroup.nodes.map((node) => (
                        ...
                        >
                          <Link href={node.href} className="group flex flex-col flex-1">
                            ...
                            {node.note && <span className="text-xs text-gray-500 mt-1 block">{node.note}</span>}
